...
const BlogCard = ({ img, date, title, id }) => {
	return (
		<Link href={`/archive/${id}`} className="flex h-[30rem] relative">
			<div className="flex bg-gray-800 w-full h-full rounded-3xl" />
			<div className="flex items-center absolute bottom-5 px-6">
				<div className="rounded-lg text-sm text-white bg-white/5 ml-1 text-center p-1">
					{date}
				</div>
